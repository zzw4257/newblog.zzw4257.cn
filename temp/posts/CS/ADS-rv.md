...
一般图上，TSP，CCP都是NPH(or C)的

metric图上TSP：

- MST任意dfs序，近似比<2 ，

- Christofides Algorithm近似比为1.5，目前为止最佳结果

  - 生成MST，将节点分成度数奇偶的O,E集， $2~\mid~ |O|$

  - 生成O上完美匹配M

  - $M\cup T$提取欧拉回路，遵循第一性原则维护不重复节点

    > $\sum (C*-e)\geqslant \sum(T)$
    >
    > $o_1,...o_k$,顺序依赖$C*$任意断边序，$C*= {C_1\oplus C_2}$，根据起始点的奇偶确定（可以手动定向），那么本质上是两个O上的完美匹配
    >
    > 则$\sum (T)+\sum(M)\leqslant (1+\frac{1}{2})\sum(C*) $最后的操作只会让左边变小
