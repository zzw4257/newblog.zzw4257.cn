...
> **📖 不可区分性在选择明文攻击下的安全性 (IND-CPA Security for Public-Key Encryption)**
    ...
    **IND-CPA 游戏 $\text{PubK}^{\text{CPA}}_{\mathcal{A},\Pi}(\lambda)$**:
    ...
    3.  **挑战密文生成 (Challenge Ciphertext Generation)**: 挑战者随机选择一个比特 $b \leftarrow_{\text{\textdollar}} \{0, 1\}$，计算挑战密文 $c^* \leftarrow \text{Enc}_{pk}(m_b)$，并将 $c^*$ 发送给 $\mathcal{A}$。
...
> **📖 判定性 Diffie-Hellman (DDH) 假设**
    ...
    **定义 2.1.2 (DDH 困难性)**:
    ...
    $$
    其中，群参数 $(\mathbb{G}, q, g)$ 由群生成算法 $\mathcal{G}(1^\lambda)$ 生成，$x,y,z \leftarrow_{\text{\textdollar}} \mathbb{Z}_q$。则称 DDH 问题在群 $\mathbb{G}$ 中是困难的。
...
> **📝 ElGamal 加密方案的 IND-CPA 安全性证明**
    ...
    *   $\text{Gen}(1^\lambda)$: 随机选择私钥 $sk \leftarrow_{\text{\textdollar}} \mathbb{Z}_q$，计算公钥 $pk = g^{sk}$。输出 $(pk, sk)$。
    *   $\text{Enc}_{pk}(m)$: 对于消息 $m \in \mathbb{G}$，随机选择 $r \leftarrow_{\text{\textdollar}} \mathbb{Z}_q$，计算密文 $c = (c_1, c_2) = (g^r, m \cdot pk^r)$。
    ...
    **证明思路 (归约)**:
    ...
    2.  $\mathcal{B}$ 模拟 ElGamal 的 IND-CPA 游戏与 $\mathcal{A}$ 交互：
        ...
        *   $\mathcal{B}$ 随机选择比特 $b \leftarrow_{\text{\textdollar}} \{0, 1\}$。
...
> *   **MPC**: Secure Multi-Party Computation。
    ...
    *   **$[n]$**: 集合 $\{1, 2, \dots, n\}$。
    *   **$\leftarrow_{\text{\textdollar}}$**: 从某个集合或分布中均匀随机选取。
    *   **$|X|$**: 集合 $X$ 的大小（基数）。
...
2.  **选择明文攻击下的不可区分性（IND-CPA Security）**：对于任意 PPT 敌手 $\mathcal{A}^{\text{Enc}_k(\cdot)}$（拥有加密预言机访问权限），存在可忽略函数 $\text{negl}(\lambda)$ 使得
    ...
    b \leftarrow_{\text{\textdollar}} \{0, 1\}; \\
